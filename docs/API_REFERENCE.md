...
### `GET /suppressions`
Lists unsubscribe events already synchronized into the Mailmerge database, newest first. The response includes `email`, immutable `campaign_id`, resolved campaign name in `campaign`, and `unsubscribed_at` for each event, plus the most recent manual synchronization time in `last_synced_at`.
