...
2. **Campaign Processing (`worker.process_campaign()`)**:
   ...
   - On success: marks `status = 'sent'`, records `sent_at`, and adds `DeliveryAttempt(outcome='sent')`. The individual message can be regenerated from the campaign templates and recipient values.
